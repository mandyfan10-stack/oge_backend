...
## 2024-05-21 - Denial of Service via Memory Exhaustion in ChatRequest
**Vulnerability:** The `history` field in `ChatRequest` lacked a length constraint, allowing an attacker to submit an unbounded array of messages, potentially causing memory exhaustion and a Denial of Service (DoS).
**Learning:** Pydantic models must have constraints on list/array fields, not just string lengths, to prevent large payload attacks.
**Prevention:** Always use `max_length` (or `max_items`) constraints on Pydantic list fields to enforce safe boundaries on incoming data sizes.
